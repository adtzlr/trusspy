    ...
    for f0i,Xi in zip(f0,X):
        ...
        if ~(f0i==np.zeros(3)).all():
            ...
            if view=='3d':
                ...
                ax = plt.gca()
                if f0i[i] != 0:
                    plt.arrow(xx,yy,scale*f0i[i],           0,color=color,linewidth=3,zorder=20,width=0.025,length_includes_head=True)
                    plt.plot([xx, xx + scale*f0i[i]], [yy, yy + 0], alpha=0)
                if f0i[j] != 0:
                    plt.arrow(xx,yy,           0,scale*f0i[j],color=color,linewidth=3,zorder=20,width=0.025,length_includes_head=True)
                    plt.plot([xx, xx + 0], [yy, yy + scale*f0i[j]], alpha=0)
    #plt.axes().set_aspect('equal');
    return fig, ax
    
def plot_elems(E,X,fig=None,ax=None,view='xz',color='C0',contour=None,lim_scale=1.2):
    ...
    for k,e in enumerate(E):
        ...
        if view=='3d':
            ...
            if ax is None: ax = fig.add_subplot(111, projection='3d')
            if contour is not None:
                ax.plot([X[NA-1][0],X[NE-1][0]],
                        [X[NA-1][1],X[NE-1][1]],
                        [X[NA-1][2],X[NE-1][2]],color='C7',zorder=1,linewidth=8.0)
            ax.plot([X[NA-1][0],X[NE-1][0]],
                    [X[NA-1][1],X[NE-1][1]],
                    [X[NA-1][2],X[NE-1][2]],color=color,zorder=2,linewidth=6.0)
        else: #2d
            if contour is not None:
                line, = plt.plot([X[NA-1][i],X[NE-1][i]],
                                 [X[NA-1][j],X[NE-1][j]],color='C7',zorder=1,linewidth=8.0)
            line, = plt.plot([X[NA-1][i],X[NE-1][i]],
