# -*- coding: utf-8 -*-
"""
Created on Wed Aug  1 12:12:56 2018
...
#                   scale=force_scale)
    ...
    if force_scale is not None and ('deformed' in config or 'undeformed' in config):
        ...
        if view == '3d':
            ax.text2D(0.3, 1.05, textstr, transform=ax.transAxes, fontsize=11,
    ...
    if len(plt.gca().yaxis.get_label().get_text()) == 0:
        ...
        str_ins = ', '
    if view=='3d':
        ax.set_title('INCREMENT: '+str(inc)+str_ins+plt.gca().yaxis.get_label().get_text())
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_zlabel('z')
    else:
        plt.title('INCREMENT: '+str(inc)+str_ins+plt.gca().yaxis.get_label().get_text())
        plt.xlabel(view[0])
        plt.ylabel(view[1])
...
def p_history(self, nodes=[1, 1], increments=None, X='Displacement X', Y='LPF', fig=None, ax=None):
    # loop over increments
    xx = [0]
    yy = [0]
    dir_dict = {'X': 0, 'Y': 1, 'Z': 2}
