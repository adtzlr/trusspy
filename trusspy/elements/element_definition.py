...
def truss(e,nodes,Xnodes,Unodes,U0nodes,rnodes,
          nodelist,elementlist,stage,
    ...
    if stage == 'K':
        ...
        analysis.K[np.where(nodelist == NA)[0][0],np.where(nodelist == NA)[0][0]] +=  KTEE # KT[ :3, :3]# KTEE
        analysis.K[np.where(nodelist == NA)[0][0],np.where(nodelist == NE)[0][0]] += -KTEE # KT[3:6, :3]# -KTEE
        analysis.K[np.where(nodelist == NE)[0][0],np.where(nodelist == NA)[0][0]] += -KTEE # KT[ :3,3:6]#-KTEE
        analysis.K[np.where(nodelist == NE)[0][0],np.where(nodelist == NE)[0][0]] +=  KTEE # KT[ :3, :3]# KTEE
