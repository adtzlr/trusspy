...
def dfdx(Vred,V0red,j,Vmax,equilibrium,stiffness,analysis=None,statev_write=False):
    ...
    analysis.Kmod = np.vstack((np.hstack((stiffness(Vred[:-1],
                                                             analysis=analysis),
                                         -analysis.f0red)),
                                  qc))
...
def dxmax_control(dxmax,dxmax0,j,j0,z,cycl,nr_success,b,n,nfev,
              minfac=1e-6,maxfac=10,reduce=8.0,increase=0.5,verbose=2):
...
def pathfollow(g,dgdx, x, analysis, dxmax=[0.02,0.02],
               j=None, j_fixed=False, j_pre=True, xlimit=None,
